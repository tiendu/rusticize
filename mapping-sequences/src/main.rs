...
fn dna_to_bits(dna: &str) -> Vec<u8> {
    ...
    dna.chars().map(|c| *nucl_to_bits.get(&c).unwrap()).collect()
}
...
fn reverse_complement(dna: &str) -> String {
    ...
    .cloned()
    .collect::<std::collections::HashMap<_, _>>();
    dna.chars().rev().map(|c| *complement.get(&c).unwrap()).collect()
}
...
fn search(
    ...
    match (text, pattern) {
        ...
        _ => panic!("Mismatched search modes: both must be DNA or General."),
    }
...
fn main() -> io::Result<()> {
    ...
    if args.len() < 3{
        eprintln!("Usage: {} <query_file> <reference_file> <report_file> [similarity_threshold] [coverage_threshold] [num_threads] [nu/aa] [circular/linear]", args[0]);
    }
    ...
    let comparison_mode = if mode == "nu" {
        ComparisonMode::Nucleotide
    } else {
        ComparisonMode::AminoAcid
    };

    if comparison_mode == ComparisonMode::AminoAcid && is_circular {
    ...
    if !Path::new(query_file).exists() {
        ...
        return Err(io::Error::new(io::ErrorKind::NotFound, "Query file not found"));
    }

    if !Path::new(reference_file).exists() {
        eprintln!("The reference file '{}' does not exists.", reference_file);
        return Err(io::Error::new(io::ErrorKind::NotFound, "Reference file not found"));
    }
